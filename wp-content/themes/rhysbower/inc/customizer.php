 ...
 */
function rhysbower_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'rhysbower_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'rhysbower' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => rhysbower_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'rhysbower' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rhysbower' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'rhysbower' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rhysbower' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rhysbower' );
}
add_action( 'customize_register', 'rhysbower_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'rhysbower_color_schemes'}.
 *
 ...
 */
function rhysbower_get_color_schemes() {
	return apply_filters( 'rhysbower_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'rhysbower' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'rhysbower_get_color_scheme' ) ) :
/**
 ...
 */
function rhysbower_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = rhysbower_get_color_schemes();
...
endif; // rhysbower_get_color_scheme

if ( ! function_exists( 'rhysbower_get_color_scheme_choices' ) ) :
/**
 ...
 */
function rhysbower_get_color_scheme_choices() {
	$color_schemes                = rhysbower_get_color_schemes();
...
endif; // rhysbower_get_color_scheme_choices

if ( ! function_exists( 'rhysbower_sanitize_color_scheme' ) ) :
/**
 ...
 */
function rhysbower_sanitize_color_scheme( $value ) {
	$color_schemes = rhysbower_get_color_scheme_choices();
...
endif; // rhysbower_sanitize_color_scheme
 ...
 */
function rhysbower_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = rhysbower_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = rhysbower_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = rhysbower_get_color_scheme_css( $colors );

	wp_add_inline_style( 'rhysbower-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'rhysbower_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function rhysbower_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', rhysbower_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'rhysbower_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function rhysbower_customize_preview_js() {
	wp_enqueue_script( 'rhysbower-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'rhysbower_customize_preview_js' );
 ...
 */
function rhysbower_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function rhysbower_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-rhysbower-color-scheme">
		<?php echo rhysbower_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'rhysbower_color_scheme_css_template' );
